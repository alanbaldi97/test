        ...
        <div v-if="$can('view airports')" class="col-12 col-md-12 q-mt-md" >
            <q-table  :data="airports" :columns="columns">
                <template v-slot:body-cell-actions="props">
                    <q-btn v-if="$can('update airport')" color="primary" size="md" icon="edit" round flat  @click="onEdit(props.row)">
                        <q-tooltip content-class="bg-indigo" :offset="[10, 10]">
                            Editar
                        </q-tooltip>
                    </q-btn>
                    <q-btn v-if="$can('delete airport')" color="red" size="md" icon="delete" round flat  @click="onDelete(props.row)">
                        <q-tooltip content-class="bg-indigo" :offset="[10, 10]">
                            Eliminar
                        </q-tooltip>
                    </q-btn>
